...
import de.gontrix.farmingcompost.common.blocks.CompostBlockEntity;
...
public class CompostMenu extends AbstractContainerMenu {
    public final CompostBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public CompostMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
        this(id, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(2));
    }

    public CompostMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
        super(Menu.COMPOST_MENU.get(), id);
        checkContainerSize(inv, 3);
        blockEntity = (CompostBlockEntity) entity;
    ...
    public @NotNull ItemStack quickMoveStack(@NotNull Player playerIn, int index) {
        ...
        if (index < VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT) {
            if (!CompostBlockEntity.isValidItem(sourceStack)) {
        ...
        } else if (index < TE_INVENTORY_FIRST_SLOT_INDEX + TE_INVENTORY_SLOT_COUNT + 1) {
    ...
    public boolean stillValid(@NotNull Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, Blocks.COMPOST.get());
    }
}
